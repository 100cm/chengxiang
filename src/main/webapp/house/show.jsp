                    ...
                    <table style="font-size: 14px;color: #34495E" class="table table-striped table-bordered table-hover" id="dataTables-example">
                        ...
                        <c:forEach items="${requestScope.houses}" var="houses" varStatus="status">
                            ...
                            <tr >
                                ...
                                <td >
                                    <c:choose>
                                        <c:when test="${'0' eq status.current.isempty}">未定</c:when>
                                        <c:when test="${'1' eq status.current.isempty}">空闲中/可出租</c:when>
                                        <c:when test="${'2' eq status.current.isempty}">出租中</c:when>
                                    </c:choose>
                                </td>
                                <td><c:choose>
                                ...
                                <button class="btn btn-xs btn-primary" onclick="empty_house('${status.current.id}')">设为空房</button>
                                </c:when>
